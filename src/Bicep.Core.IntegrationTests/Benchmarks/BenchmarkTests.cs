...
{
    private const int EventCounterIntervalSec = 1;
    ...
    public async Task Benchmarks_should_match_expected(DataSet dataSet)
    {
        ...
        var program = new Program(new(new StringWriter(), new StringWriter()));
        ...
        perfFile.WriteToOutputFolder(FormatBaselineFile(bicepCounters));
    ...
    public class SimpleEventListener : EventListener
    {
        ...
        {
            if (source.Name != BicepEventSource.EventSourceName &&
                source.Name != "Microsoft-Windows-DotNETRuntime")
            {
                return;
            }
            ...
            {
                ["EventCounterIntervalSec"] = $"{EventCounterIntervalSec}",
            });
        }
        ...
        {
            if (eventData.EventSource.Name == BicepEventSource.EventSourceName &&
