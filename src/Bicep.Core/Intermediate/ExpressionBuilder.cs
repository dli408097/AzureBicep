...
public class ExpressionBuilder
{
    ...
    private ObjectProperty ConvertObjectProperty(ObjectPropertySyntax syntax)
    {
        ...
        return new(keyExpression, Convert(syntax.Value));
    }
                ...
                switch (baseSymbol)
                {
                    ...
                    case { } _ when context.SemanticModel.ResourceMetadata.TryLookup(baseSyntax) is DeclaredResourceMetadata resource:
                        if (method.Name.IdentifierName.StartsWithOrdinalInsensitively(LanguageConstants.ListFunctionPrefix))
                        {
                            ...
                            var resourceIdExpression = new ResourceIdExpression(baseSyntax, resource, indexContext);
